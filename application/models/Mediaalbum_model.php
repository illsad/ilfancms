...
class Mediaalbum_model extends CI_Model {
    ...
    public function gets($offset, $keyword = '') {
        ...
        $sql = 'SELECT 
				   mediamanager_album.id, mediamanager_album.label, mediamanager_album.upload_at,
				   (SELECT COUNT(*) FROM mediamanager mm WHERE mm.album_id = mediamanager_album.id) count
				FROM
				   mediamanager_album
				ORDER BY
				   mediamanager_album.upload_at DESC '.
        		$limit;
